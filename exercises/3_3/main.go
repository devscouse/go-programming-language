...
// are colored red (#ff0000) and the valleys are colored blue (#0000ff)
package main
...
func main() {
	...
	for i := range cells {
		for j := range cells {
			...
			fmt.Printf(
				"<polygon points='%g,%g,%g,%g,%g,%g,%g,%g' fill='#%02x%02x%02x'/>\n",
...
func polygonColor(i int, j int) color.RGBA {
